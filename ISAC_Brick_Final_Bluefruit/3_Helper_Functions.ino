...
void button_handler() {
  ...
  if ( (digitalRead(BOTTOM_BUTTON) || digitalRead(TOP_BUTTON)) && both_buttons_pressed) {  //if either button released and both were pressed
    ...
    if ( (millis() - both_pressed_time) > 2000) {
      try_to_connect = true;  //set flag to try and connect in the bt_handler
      bt_scan_start = millis();  //note when we started trying to connect
    }
  }
...
//handles all the bluetooth related communication things
//when to be connectable, how to change state flags based on what the bluetooth input is, etc.
//PARAMS - none
//RETURNS - none
void bt_handler(){
  if(!bt.isConnected() && try_to_connect){
    if(!connectable){  //if nothing can connect to us
      bt.atcommand(F("AT+GAPCONNECTABLE"), (int32_t) 1);  //make us connectable
      bt.atcommand(F("AT+BLEPOWERLEVEL"), (int32_t) -20);// up the transmit power to make connecting easier
      connectable = true;  //update state flag appropraitely
      queue_audio_file(ESTABLISHING_UPLINK);
    }
    if( (millis() - bt_scan_start) > 10000){  //if 10s connectable and bt hasn't connected
      try_to_connect = false;  //we are no longer trying to connect
      bt.atcommand(F("AT+BLEPOWERLEVEL"), (int32_t) -20);  //turn down power 
      bt.atcommand(F("AT+GAPCONNECTABLE"), (int32_t) 0);  //turn off the ability to connect
      connectable = false;  //update state flag accordingly
      queue_audio_file(PROXIMITY_COVERAGE_ONLY);
    }
  }

  if( (!bt.isConnected()&&last_connected) || (bt.isConnected()&&last_connected&&try_to_connect) ) {  //if we just disconnected || if we initiated a disconnect
    last_connected = false;  //update state flags
    connectable = false;
    try_to_connect = false;
    bt.atcommand(F("AT+GAPDISCONNECT"));
    bt.atcommand(F("AT+GAPCONNECTABLE"), (int32_t) 0);  //turn off the ability to connect
    bt.atcommand(F("AT+BLEPOWERLEVEL"), (int32_t) -40);//turn the power back to lowest
    queue_audio_file(NETWORK_REVOKED);  //alert user
    constant_bt_data = false;  //make sure we don't send bt data constantly on a new connection
  }
  
  if(bt.isConnected()){
    if(try_to_connect == true){  //if we are connected and were just trying to connect
      try_to_connect = false;  //we just connected, update state flag
      last_connected = true;  //update another state flag
      queue_audio_file(ACCESSING_NETWORK);  //and play appropriate audio file
    }
    if(bt.available()){
      char bt_data = (char) bt.read();
      switch(bt_data){
        case 'o':  //read data to bt once
          bt_log_data();
          break;
        case 'c':  //read data to bt repeatedly
          constant_bt_data = !constant_bt_data;
          break;
        case 'h':  //swap bt power to high
          bt.println("BT High Power");
          bt.atcommand(F("AT+BLEPOWERLEVEL"), (int32_t) 4);
          break;
        case 'l':  //swap bt power to low
          bt.println("BT Low Power");
          bt.atcommand(F("AT+BLEPOWERLEVEL"), (int32_t) -40);
          break;
        default:
          bt.println("read data Once, read data Constant, set to High power, set to Low power");
          break;
      }
...
//formats a few specific pieces of data into an easily human-readable format and sends over bluetooth
//PARAMS - None
//RETURNS - None
void bt_log_data(){
  String data_string = "TVOC: ";
...
void alert_handler(byte _last_active_alerts, byte _active_alerts) {
  ...
  if (bitRead(diff_alerts, 3) ){  //if bt init error
    queue_audio_file(TRANSMISSION_JAMMED);  //alert user
  }
