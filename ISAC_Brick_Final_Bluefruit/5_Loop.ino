...
void loop(){
  ...
  if( (millis()-last_scan) > scan_rate){
    ...
    //send data over bluetooth if available and asked for
    if(bt.isConnected() && constant_bt_data){
      bt_log_data();
    }
  ...
  if( (millis()-last_alert_time) > alert_rate){  //if it's been a minute since last alert
    if(bitRead(active_alerts, 4) && !charging ){  //if battery low and we're not charging (to endless warnings while charging
